...
async def imdb_callback(bot: Client, query: CallbackQuery):
    ...
    if imdb.get('poster'):
        ...
        await query.message.edit(f"IMDb Data:\n\n<b>🎬 𝖳𝗂𝗍𝗅𝖾 : <a href={imdb['url']}>{imdb.get('title')}</a></b>\n<b>🎭 Genres : {imdb.get('genres')}</b>\n<b>📆 Year : <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a></b>\n<b>🌟 Rating : <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> </b>/ 10\n<b>🖋 StoryLine : {imdb.get('plot')} </b>", reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)
